...
enum EventProvider {
    ...
    case putEvent(eventId: Int, title: String, imageUrl: String, maxMember: Int, place: Int, genreId: Int)
    ...
    var path: String {
        switch self {
            case .getEvents, .postEvent:
                return "/events"
            case .putEvent(let eventId, _, _, _, _, _):
    ...
    var sampleData: Data {
        switch self {
            ...
            case .getParticipantEvents:
                ...
                return FileHandle(forReadingAtPath: path)!.readDataToEndOfFile()
            case .postApplyEvent, .postCancelEvent, .postFavorite, .deleteEvent:
                return .empty
            }
    }
    ...
    var task: Task {
        switch self {
            ...
            case .postEvent(let title, let imageUrl, let maxMember, let place, let genreId),
                    .putEvent(_, let title, let imageUrl, let maxMember, let place, let genreId):
            ...
            case .postCancelEvent(_, let prefectureId):
                return .requestParameters(parameters: ["prefecture_id": prefectureId], encoding: URLEncoding.default)
            case .getEventDetails, .getFavoriteEvents, .getParticipantEvents, .postFavorite, .deleteEvent:
                return .requestPlain
        }
    }
